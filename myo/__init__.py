...
# Edit for recording two Myos with audio by Qichao Lan and Cagri Erdem, 2019.
#
    ...
    while time.time() - t < 1.0:  # todo, put a timeout here.
        ...
        if uuid == MyoServiceInfoUuid:  # This is MYO_SERVICE_INFO_UUID # found a myo.
            ...
            if name not in myo_details.keys():
                print(
                    "Found a Myo:", name, "MAC:", mac_address_string
                )  # print the Myo's mac address
            myo_details[
                name
            ] = mac_address_string  # this can be wrong if there are multiple myos -- Qichao
    adapter.end_scan()
    return myo_details
...
class Myo(object):
    ...
    def __init__(self, adapter=None, tty=None, start_time=None):
    ...
    def find_mac(self):
        ...
        try:
            address = list(myo_details.values())  # no need to stick to 0 here
        except IndexError:
            print("No Myos found to connect to.")
            print("Not connected.")
        return address  # Todo: how to make sure the order?
    ...
    def connect(self, address=None):
        ...
        if address is None:
            myo_details = discover_myos(self.bt)
            try:
                address = list(myo_details.values())[
                    0
                ]  # Could fail if no Myo is found.
            except IndexError:
                print("No Myos found to connect to.")
                print("Not connected.")
                return
        ...
        self.write_attr(
            MyoChars.IMUDataDescriptor.value, b"\x01\x00"
        )  # enable IMU data
        self.write_attr(
            MyoChars.ArmDescriptor.value, b"\x02\x00"
        )  # enable on/off arm notifications
        self.write_attr(
            MyoChars.EmgData0Descriptor.value, b"\x01\x00"
        )  # Suscribe to EmgData0Characteristic
        self.write_attr(
            MyoChars.EmgData1Descriptor.value, b"\x01\x00"
        )  # Suscribe to EmgData1Characteristic
        self.write_attr(
            MyoChars.EmgData2Descriptor.value, b"\x01\x00"
        )  # Suscribe to EmgData2Characteristic
        self.write_attr(
            MyoChars.EmgData3Descriptor.value, b"\x01\x00"
        )  # Suscribe to EmgData3Characteristic
        self.write_attr(
            MyoChars.BatteryDescriptor.value, b"\x01\x10"
    ...
    def handle_ble_data(self, p):
        ...
        pay = p.payload[
            5:
        ]  # chop off the first five bytes of the payload for some reason.
    ...
    def emg_to_csv(self):
        for i, value in enumerate(self.emg_index):
            if value is 0:
                try:
                    self.emg_index[i] = (
                        self.emg_index[i - 1] + self.emg_index[i + 1]
                    ) / 2
                except:
                    self.emg_index[i] += (
                        self.emg_index[i - 1]
        ...
        e = pd.DataFrame(
            self.emg_data,
            index=self.emg_index,
            columns=["emg" + str(i + 1) for i in range(8)],
        )

        e.to_csv("./data/emg_" + str(self.address).replace(":", "-") + ".csv")

    def imu_to_csv(self):
        imu = pd.DataFrame(
            self.imu_data,
            index=self.imu_index,
            columns=[
                "quat1",
                "quat2",
                "quat3",
        ...
        # columns=['imu'+str(i+1) for i in range(10)])

        imu.to_csv("./data/imu_" + str(self.address).replace(":", "-") + ".csv")
    ...
    def sleep_mode(self, mode):
        ...
        self.write_attr(
            MyoChars.CommandCharacteristic.value, command_set_sleep_mode(mode)
        )
    ...
    def add_pose_handler(self, h):  # line 193
    ...
    def on_emg(self, emg_input_data):
        ...
        if self.recording is 1:
            t = time.time() - self.TIME
            self.emg_index += [t, 0]
            self.emg_data += [emg1, emg2]
